...
3. **Two LLM modes:**
   ...
   - **Conversational chat** — `lib/agents/longevity-chat.ts::runLongevityChat`. Used by `POST /api/longevity/message`. Multi-turn tool-calling agent loop with `LONGEVITY_TOOL_DEFINITIONS` (separate registry from coach-tools): `getRecentBiomarkers`, `getMarkerHistory`, `getRecentWorkouts`, `getLongevitySoul`, `updateLongevitySoul`, `getInjuryHistory`, `runDeterministicPrioritization`. Conversation persists at `daily_summaries.summary.longevityConversation` (last 20 turns). `getRecentWorkouts` reads the already-loaded `AthleteContext.recentWorkouts` (the route uses the same `loadAthleteContext` as the coach) and returns per-session detail plus an aggregate training-load summary (duration / distance / vert / kcal / load / hard-session count) so the guru grounds nutrition / fueling / hydration / recovery advice in real expenditure rather than generic guidance.
