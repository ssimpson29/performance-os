...
| `oura-probe-api.mjs [email]` | Calls the Oura API directly with the stored access token to check whether it still works (HTTP status) and what fields the `sleep` endpoint returns. Read-only. |
| `oura-sync-once.mjs [email] [days]` | Manual one-shot sync mirroring `lib/oura/recovery-sync.ts` — backfills `recovery_daily` for the last N days (default 35) and bumps `last_synced_at`. Idempotent. Use when the scheduled cron hasn't run. |
